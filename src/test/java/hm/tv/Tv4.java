...
public class Tv4 {
    ...
    public static void main(String[] args) {
        final String domain = "http://iptv807.com/";
        ...
        list.forEach((key, value) -> {
            ...
            for (Element element : aClass) {
                ...
                if (attr.contains("act=play")) {
                    ...
                    //播放链接
                    String m3u8Url = getM3u8Url(domain + attr);
                    //播放实际链接
                    Connection.Response response = null;
                    try {
                        response = Jsoup.connect(m3u8Url).timeout(5_000).ignoreContentType(true).followRedirects(true).execute();
                        System.out.println(response.body());
                        m3u8Url = response.url().toString();
